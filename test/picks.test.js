...
const HEAD = "name,address,kind,designer,year,why,hannah_note,designer_words," +
             "designer_name,link,socials,afterparty,show";
...
test("afterparty=Y puts a pick on the ballot", () => {
  withSheet(`${HEAD}\nA,,bar,,,,,,,,,Y,\nB,,bar,,,,,,,,,,`,
    { venues: [] }, (build) => {
      const out = build();
      assert.deepStrictEqual(out.afterparty.map((p) => p.name), ["A"]);
    });
});
...
test("the real spreadsheet parses, and most of it is on the map", () => {
  ...
  assert.ok(out.count > 100, `expected 100+ picks, got ${out.count}`);
  assert.ok(out.mappedCount / out.count > 0.9,
            `only ${out.mappedCount}/${out.count} are placed`);
  assert.ok(out.afterparty.length > 0, "nothing is on the afterparty ballot");
